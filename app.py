...
def api_generate_direct_project(wilaya_code):
    ...
    try:
        ...
        for raw_idea in projets_bruts:
            ...
            # ⚠️ MODIF : un PDF DÉDIÉ par projet (et non plus un seul PDF combiné
            # partagé par tous les projets de la wilaya), pour que le bouton PDF
            # de chaque ligne du dashboard télécharge uniquement son propre projet.
            rel_path = render_reco_pdf(current_user, w, full_data, [pdf_idea], suffix=str(p.id))
            p.pdf_path = rel_path
...
def render_reco_pdf(user: "User", w: "Wilaya", d: dict, ideas: list, suffix: str = "") -> str:
    reports_dir = os.path.join(app.static_folder, "reports")
    os.makedirs(reports_dir, exist_ok=True)
    # ⚠️ MODIF : microsecondes + suffixe (ex: id du projet) pour garantir un nom
    # de fichier unique même quand plusieurs PDF sont générés dans la même seconde
    # (cas typique : 3-5 projets créés coup sur coup lors d'un même appel IA).
    ts = datetime.utcnow().strftime('%Y%m%d_%H%M%S%f')
    suffix_part = f"_{suffix}" if suffix else ""
    fname = f"reco_{w.code}{suffix_part}_{ts}.pdf"
